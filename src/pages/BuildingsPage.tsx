          ...
          <div className="flex flex-col">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
              {buildings.map((building: any) => (
                <div key={building.id} className="bg-white/50 dark:bg-zinc-950/50 border border-zinc-200 dark:border-zinc-800 rounded-xl overflow-hidden hover:border-emerald-500/50 transition-all hover:shadow-lg hover:shadow-emerald-500/10 flex flex-col group">
                  <Link to={`/buildings/${building.id}`} className="block flex-1 flex flex-col group/link">
                    {/* Image Placeholder */}
                    <div className="h-48 bg-zinc-50 dark:bg-zinc-900 flex items-center justify-center relative overflow-hidden border-b border-zinc-200 dark:border-zinc-800">
                      <img 
                        src={`https://picsum.photos/seed/${building.id}/400/200`} 
                        alt={building.name}
                        className="absolute inset-0 w-full h-full object-cover opacity-50 group-hover/link:opacity-70 transition-opacity duration-500"
                        onError={(e) => {
                          e.currentTarget.style.display = 'none';
                        }}
                      />
                      <div className="absolute inset-0 bg-gradient-to-t from-zinc-950/90 to-zinc-950/20 z-10" />
                      <Building2 className="text-zinc-500/50 group-hover/link:scale-110 transition-transform duration-500 z-10" size={64} />
                      <span className="absolute bottom-3 left-4 z-20 text-xs font-medium text-emerald-400 uppercase tracking-wider bg-emerald-500/10 border border-emerald-500/20 px-2 py-1 rounded backdrop-blur-md">
                    ...
                    <div className="p-5 flex-1 flex flex-col relative z-20">
                      <h3 className="text-xl font-bold text-zinc-900 dark:text-zinc-100 truncate group-hover/link:text-emerald-400 transition-colors" title={building.name}>
