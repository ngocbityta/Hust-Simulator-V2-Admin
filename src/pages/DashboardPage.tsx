    ...
    <div className="p-8">
      <div className="mb-8 border-b border-zinc-800 pb-4 flex justify-between items-end">
        <div>
          ...
          <p className="text-zinc-400 mt-2">Các chỉ số mô phỏng theo thời gian thực và dữ liệu lịch sử</p>
        </div>
        <select 
          value={timeRange} 
          onChange={(e) => setTimeRange(e.target.value)}
          className="bg-zinc-900 border border-zinc-700 text-white text-sm rounded-lg focus:ring-emerald-500 focus:border-emerald-500 block p-2.5 outline-none cursor-pointer"
        >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
        ...
        <div className="bg-zinc-900/80 border border-zinc-800/80 rounded-2xl p-6 shadow-xl">
          <h2 className="text-lg font-semibold text-white mb-6">Mật độ bản đồ</h2>
        ...
        <div className="bg-zinc-900/80 border border-zinc-800/80 rounded-2xl p-6 shadow-xl">
          <h2 className="text-lg font-semibold text-white mb-6">Hành vi sinh viên</h2>
          <div className="h-[300px]">
            <ResponsiveContainer width="100%" height="100%">
              <PieChart>
                <Pie
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
        {/* Top 5 Busiest Nodes */}
        <div className="bg-zinc-900/80 border border-zinc-800/80 rounded-2xl p-6 shadow-xl lg:col-span-1">
          <h2 className="text-lg font-semibold text-white mb-6">Top 5 Nút giao Đông nhất</h2>
          <div className="space-y-4">
            {topNodesData.map((item: any, index: number) => (
              <div key={index} className="flex items-center justify-between p-3 rounded-xl bg-zinc-950/50 border border-zinc-800/50">
                <div>
        ...
        <div className="bg-zinc-900/80 border border-zinc-800/80 rounded-2xl p-6 shadow-xl lg:col-span-2">
          <h2 className="text-lg font-semibold text-white mb-6">Công suất Phòng học theo Giảng đường</h2>
      ...
      <div className="grid grid-cols-1 gap-6">
        {/* Upcoming Timeline */}
        <div className="bg-zinc-900/80 border border-zinc-800/80 rounded-2xl p-6 shadow-xl">
          <h2 className="text-lg font-semibold text-white mb-6">Sắp tới & Đang diễn ra</h2>
          <div className="space-y-4">
            {(d.eventsTimeline || []).slice(0, 5).map((event: any) => (
              <div key={event.id} className="relative pl-6 pb-4 border-l border-zinc-800 last:border-0 last:pb-0">
                ...
                <div className="bg-zinc-950/50 border border-zinc-800/50 rounded-xl p-3">
                  <div className="flex justify-between items-start mb-1">
                    <h3 className="text-zinc-200 font-medium">{event.name}</h3>
...
const StatCard = ({ title, value }: { title: string, value: string | number }) => (
  <div className="bg-zinc-900/80 border border-zinc-800/80 rounded-2xl p-6 shadow-lg flex flex-col justify-center items-center text-center transition-all hover:bg-zinc-800/50 cursor-default">
    <p className="text-zinc-400 text-sm font-medium uppercase tracking-wider mb-2">{title}</p>
    <h3 className="text-3xl font-bold text-white bg-clip-text text-transparent bg-gradient-to-r from-zinc-100 to-zinc-400">{value}</h3>
  </div>
);
